...
*Setup instructions for a gitian build of Eternity using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Eternity
...
- [Building Eternity](#building-eternity)
...
Clone the git repositories for eternity and gitian and then checkout the eternity version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the eternity repository
...
To build Eternity (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the eternity repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/eternity/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eternity-group/eternity
...
For example:
```bash
URL=https://github.com/crowning-/eternity.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit eternity=${COMMIT} --url eternity=${URL} ../eternity/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit eternity=${COMMIT} --url eternity=${URL} ../eternity/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit eternity=${COMMIT} --url eternity=${URL} ../eternity/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/eternity-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/eternity-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/eternity-build.assert
```
...
[eternity/gitian.sigs](https://github.com/eternity-group/gitian.sigs/) repository, or if that's not possible to create a pull
request.
