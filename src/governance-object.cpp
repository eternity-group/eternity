...
: cs(),
  ...
  vinEternitynode(other.vinEternitynode),
  vchSig(other.vchSig),
...
{
    int nMNIndex = governance.GetEternitynodeIndex(vote.GetVinEternitynode());
    if(nMNIndex < 0) {
        std::ostringstream ostr;
        ostr << "CGovernanceObject::ProcessVote -- Eternitynode index not found\n";
        exception = CGovernanceException(ostr.str(), GOVERNANCE_EXCEPTION_WARNING);
        if(mapOrphanVotes.Insert(vote.GetVinEternitynode(), vote_time_pair_t(vote, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME))) {
            if(pfrom) {
                mnodeman.AskForEN(pfrom, vote.GetVinEternitynode());
            }
    ...
    if(governance.AreRateChecksEnabled()) {
        ...
        if(nTimeDelta < GOVERNANCE_UPDATE_MIN) {
            std::ostringstream ostr;
            ostr << "CGovernanceObject::ProcessVote -- Eternitynode voting too often "
    ...
    if(!mnodeman.AddGovernanceVote(vote.GetVinEternitynode(), vote.GetParentHash())) {
        std::ostringstream ostr;
...
void CGovernanceObject::RebuildVoteMap()
{
    ...
    for(vote_m_it it = mapCurrentMNVotes.begin(); it != mapCurrentMNVotes.end(); ++it) {
        ...
        if(mnodeman.GetEternitynodeVinForIndexOld(it->first, vinEternitynode)) {
            int nNewIndex = mnodeman.GetEternitynodeIndex(vinEternitynode);
            if((nNewIndex >= 0)) {
...
void CGovernanceObject::ClearEternitynodeVotes()
{
    ...
    while(it != mapCurrentMNVotes.end()) {
        ...
        CTxIn vinEternitynode;
        bool fRemove = true;
        if(mnodeman.Get(it->first, vinEternitynode, fIndexRebuilt)) {
            if(mnodeman.Has(vinEternitynode)) {
                fRemove = false;
            }
            else {
                fileVotes.RemoveVotesFromEternitynode(vinEternitynode);
            }
        }
...
void CGovernanceObject::SetEternitynodeInfo(const CTxIn& vin)
{
    vinEternitynode = vin;
}

bool CGovernanceObject::Sign(CKey& keyEternitynode, CPubKey& pubKeyEternitynode)
{
    std::string strError;
    ...
    LogPrint("gobject", "CGovernanceObject::Sign -- pubkey id = %s, vin = %s\n",
             pubKeyEternitynode.GetID().ToString(), vinEternitynode.prevout.ToStringShort());


    return true;
}

bool CGovernanceObject::CheckSignature(CPubKey& pubKeyEternitynode)
{
    std::string strError;
...
{
    bool fMissingEternitynode = false;

    return IsValidLocally(strError, fMissingEternitynode, fCheckCollateral);
}

bool CGovernanceObject::IsValidLocally(std::string& strError, bool& fMissingEternitynode, bool fCheckCollateral)
{
    fMissingEternitynode = false;
    ...
    // IF ABSOLUTE NO COUNT (NO-YES VALID VOTES) IS MORE THAN 10% OF THE NETWORK ETERNITYNODES, OBJ IS INVALID

    // CHECK COLLATERAL IF REQUIRED (HIGH CPU USAGE)

    if(fCheckCollateral) { 
        if((nObjectType == GOVERNANCE_OBJECT_TRIGGER) || (nObjectType == GOVERNANCE_OBJECT_WATCHDOG)) {
            std::string strOutpoint = vinEternitynode.prevout.ToStringShort();
            eternitynode_info_t infoMn = mnodeman.GetEternitynodeInfo(vinEternitynode);
            if(!infoMn.fInfoValid) {
                fMissingEternitynode = true;
                strError = "Eternitynode not found: " + strOutpoint;
                return false;
            }

            // Check that we have a valid MN signature
            if(!CheckSignature(infoMn.pubKeyEternitynode)) {
                strError = "Invalid eternitynode signature for: " + strOutpoint + ", pubkey id = " + infoMn.pubKeyEternitynode.GetID().ToString();
                return false;
            }
...
{
    int nMNIndex = governance.GetEternitynodeIndex(mnCollateralOutpoint);
    vote_m_it it = mapCurrentMNVotes.find(nMNIndex);
    if (it == mapCurrentMNVotes.end()) {
        return false;
    }
...
void CGovernanceObject::CheckOrphanVotes()
{
    ...
    while(it != listVotes.end()) {
        ...
        else if(!mnodeman.Has(vote.GetVinEternitynode())) {
            ++it;
            continue;
        }
        CGovernanceException exception;
