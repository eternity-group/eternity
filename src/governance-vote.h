...
// INTENTION OF ETERNITYNODES REGARDING ITEM
...
//
// CGovernanceVote - Allow a eternitynode node to vote and broadcast throughout the network
//
...
public:
    CGovernanceVote();
    CGovernanceVote(CTxIn vinEternitynodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn);
    ...
    CTxIn& GetVinEternitynode() { return vinEternitynode; }

    const CTxIn& GetVinEternitynode() const { return vinEternitynode; }
    ...
    *   GetTypeHash()
    *
    *   GET HASH WITH DETERMINISTIC VALUE OF ETERNITYNODE-VIN/PARENT-HASH/VOTE-SIGNAL
    *
    *   This hash collides with previous eternitynode votes when they update their votes on governance objects.
    ...
    *   --
    *
    *   We do not include an outcome, because that can change when a eternitynode updates their vote from yes to no
