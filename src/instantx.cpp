...
//step 2.) Top INSTANTX_SIGNATURES_TOTAL eternitynodes, open connect to top 1 eternitynode.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 eternitynode, waits for INSTANTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
        ...
        if(ProcessConsensusVote(pfrom, ctx)){
            ...
            */
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                if(!mapUnknownVotes.count(ctx.vinEternitynode.prevout.hash)){
                    mapUnknownVotes[ctx.vinEternitynode.prevout.hash] = GetTime()+(60*10);
                }

                if(mapUnknownVotes[ctx.vinEternitynode.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinEternitynode.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::ix - eternitynode is spamming transaction votes: %s %s\n",
                            ctx.vinEternitynode.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which eternitynodes
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown Eternitynode\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Eternitynode not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinEternitynode = activeEternitynode.vin;
...
bool ProcessConsensusVote(CNode* pnode, CConsensusVote& ctx)
{
    ...
    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Eternitynode not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }
...
{
    return vinEternitynode.prevout.hash + vinEternitynode.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if(pen == NULL)
    {
        LogPrintf("InstantX::CConsensusVote::SignatureValid() - Unknown Eternitynode\n");
        return false;
    }

    if(!spySendSigner.VerifyMessage(pen->pubkey2, vchEternityNodeSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strEternityNodePrivKey.c_str());

    if(!spySendSigner.SetKey(strEternityNodePrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid eternitynodeprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!spySendSigner.SignMessage(strMessage, errorMessage, vchEternityNodeSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if(!spySendSigner.VerifyMessage(pubkey2, vchEternityNodeSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    ...
    BOOST_FOREACH(CConsensusVote vote, vecConsensusVotes)
    {
        ...
        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown Eternitynode\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Eternitynode not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
