...
// step 2) Top COutPointLock::SIGNATURES_TOTAL eternitynodes per each spent outpoint push "txvote" message
...
void CInstantSend::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Ignore any InstantSend messages until eternitynode list is synced
...
bool CInstantSend::ProcessTxLockRequest(const CTxLockRequest& txLockRequest)
{
    ...
    // Eternitynodes will sometimes propagate votes before the transaction is known to the client.
...
void CInstantSend::Vote(CTxLockCandidate& txLockCandidate)
{
    ...
    while(itOutpointLock != txLockCandidate.mapOutPointLocks.end()) {
        ...
        int n = mnodeman.GetEternitynodeRank(activeEternitynode.vin, nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

        if(n == -1) {
            LogPrint("instantsend", "CInstantSend::Vote -- Unknown Eternitynode %s\n", activeEternitynode.vin.prevout.ToStringShort());
            ++itOutpointLock;
            continue;
        }
        ...
        if(n > nSignaturesTotal) {
            LogPrint("instantsend", "CInstantSend::Vote -- Eternitynode not in the top %d (%d)\n", nSignaturesTotal, n);
            ++itOutpointLock;
            continue;
        }
        ...
        if(itVoted != mapVotedOutpoints.end()) {
            BOOST_FOREACH(const uint256& hash, itVoted->second) {
                ...
                if(it2->second.HasEternitynodeVoted(itOutpointLock->first, activeEternitynode.vin.prevout)) {
        ...
        CTxLockVote vote(txHash, itOutpointLock->first, activeEternitynode.vin.prevout);
...
bool CInstantSend::ProcessTxLockVote(CNode* pfrom, CTxLockVote& vote)
{
    ...
    if(it == mapTxLockCandidates.end()) {
        if(!mapTxLockVotesOrphan.count(vote.GetHash())) {
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  eternitynode=%s new\n",
                    txHash.ToString(), vote.GetEternitynodeOutpoint().ToStringShort());
            ...
            LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- Orphan vote: txid=%s  eternitynode=%s seen\n",
                    txHash.ToString(), vote.GetEternitynodeOutpoint().ToStringShort());
        }
        ...
        int nEternitynodeOrphanExpireTime = GetTime() + 60*10; // keep time data for 10 minutes
        if(!mapEternitynodeOrphanVotes.count(vote.GetEternitynodeOutpoint())) {
            mapEternitynodeOrphanVotes[vote.GetEternitynodeOutpoint()] = nEternitynodeOrphanExpireTime;
        } else {
            int64_t nPrevOrphanVote = mapEternitynodeOrphanVotes[vote.GetEternitynodeOutpoint()];
            if(nPrevOrphanVote > GetTime() && nPrevOrphanVote > GetAverageEternitynodeOrphanVoteTime()) {
                LogPrint("instantsend", "CInstantSend::ProcessTxLockVote -- eternitynode is spamming orphan Transaction Lock Votes: txid=%s  eternitynode=%s\n",
                        txHash.ToString(), vote.GetEternitynodeOutpoint().ToStringShort());
    ...
    if(it1 != mapVotedOutpoints.end()) {
        BOOST_FOREACH(const uint256& hash, it1->second) {
            if(hash != txHash) {
                ...
                std::map<uint256, CTxLockCandidate>::iterator it2 = mapTxLockCandidates.find(hash);
                if(it2->second.HasEternitynodeVoted(vote.GetOutpoint(), vote.GetEternitynodeOutpoint())) {
                    // yes, it did, refuse to accept a vote to include the same outpoint in another tx
                    // from the same eternitynode.
                    // TODO: apply pose ban score to this eternitynode?
                    ...
                    // pose ban score too.
                    LogPrintf("CInstantSend::ProcessTxLockVote -- eternitynode sent conflicting votes! %s\n", vote.GetEternitynodeOutpoint().ToStringShort());
                    return false;
                }
            }
        }
        // we have votes by other eternitynodes only (so far), let's continue and see who will win
...
int64_t CInstantSend::GetAverageEternitynodeOrphanVoteTime()
{
    LOCK(cs_instantsend);
    // NOTE: should never actually call this function when mapEternitynodeOrphanVotes is empty
    if(mapEternitynodeOrphanVotes.empty()) return 0;

    std::map<COutPoint, int64_t>::iterator it = mapEternitynodeOrphanVotes.begin();
    int64_t total = 0;

    while(it != mapEternitynodeOrphanVotes.end()) {
        total+= it->second;
        ++it;
    }

    return total / mapEternitynodeOrphanVotes.size();
}
...
void CInstantSend::CheckAndRemove()
{
    ...
    while(itVote != mapTxLockVotes.end()) {
        if(itVote->second.IsExpired(pCurrentBlockIndex->nHeight)) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired vote: txid=%s  eternitynode=%s\n",
                    itVote->second.GetTxHash().ToString(), itVote->second.GetEternitynodeOutpoint().ToStringShort());
    ...
    while(itOrphanVote != mapTxLockVotesOrphan.end()) {
        if(GetTime() - itOrphanVote->second.GetTimeCreated() > ORPHAN_VOTE_SECONDS) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan vote: txid=%s  eternitynode=%s\n",
                    itOrphanVote->second.GetTxHash().ToString(), itOrphanVote->second.GetEternitynodeOutpoint().ToStringShort());
    ...
    // remove expired eternitynode orphan votes (DOS protection)
    std::map<COutPoint, int64_t>::iterator itEternitynodeOrphan = mapEternitynodeOrphanVotes.begin();
    while(itEternitynodeOrphan != mapEternitynodeOrphanVotes.end()) {
        if(itEternitynodeOrphan->second < GetTime()) {
            LogPrint("instantsend", "CInstantSend::CheckAndRemove -- Removing expired orphan eternitynode vote: eternitynode=%s\n",
                    itEternitynodeOrphan->first.ToStringShort());
            mapEternitynodeOrphanVotes.erase(itEternitynodeOrphan++);
        } else {
            ++itEternitynodeOrphan;
        }
    }
}
...
{
    if(!mnodeman.Has(CTxIn(outpointEternitynode))) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Unknown eternitynode %s\n", outpointEternitynode.ToStringShort());
        mnodeman.AskForEN(pnode, CTxIn(outpointEternitynode));
        return false;
    }
    ...
    int n = mnodeman.GetEternitynodeRank(CTxIn(outpointEternitynode), nLockInputHeight, MIN_INSTANTSEND_PROTO_VERSION);

    if(n == -1) {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantsend", "CTxLockVote::IsValid -- Outdated eternitynode %s\n", outpointEternitynode.ToStringShort());
        return false;
    }
    LogPrint("instantsend", "CTxLockVote::IsValid -- Eternitynode %s, rank=%d\n", outpointEternitynode.ToStringShort(), n);

    int nSignaturesTotal = COutPointLock::SIGNATURES_TOTAL;
    if(n > nSignaturesTotal) {
        LogPrint("instantsend", "CTxLockVote::IsValid -- Eternitynode %s is not in the top %d (%d), vote hash=%s\n",
                outpointEternitynode.ToStringShort(), nSignaturesTotal, n, GetHash().ToString());
        return false;
    }
...
bool CTxLockVote::CheckSignature() const
{
    ...
    eternitynode_info_t infoMn = mnodeman.GetEternitynodeInfo(CTxIn(outpointEternitynode));

    if(!infoMn.fInfoValid) {
        LogPrintf("CTxLockVote::CheckSignature -- Unknown Eternitynode: eternitynode=%s\n", outpointEternitynode.ToString());
        return false;
    }

    if(!spySendSigner.VerifyMessage(infoMn.pubKeyEternitynode, vchEternitynodeSignature, strMessage, strError)) {
...
bool CTxLockVote::Sign()
{
    ...
    if(!spySendSigner.SignMessage(strMessage, vchEternitynodeSignature, activeEternitynode.keyEternitynode)) {
        LogPrintf("CTxLockVote::Sign -- SignMessage() failed\n");
        return false;
    }

    if(!spySendSigner.VerifyMessage(activeEternitynode.pubKeyEternitynode, vchEternitynodeSignature, strMessage, strError)) {
...
{
    if(mapEternitynodeVotes.count(vote.GetEternitynodeOutpoint()))
        return false;
    mapEternitynodeVotes.insert(std::make_pair(vote.GetEternitynodeOutpoint(), vote));
    return true;
}
...
std::vector<CTxLockVote> COutPointLock::GetVotes() const
{
    ...
    std::map<COutPoint, CTxLockVote>::const_iterator itVote = mapEternitynodeVotes.begin();
    while(itVote != mapEternitynodeVotes.end()) {
...
bool COutPointLock::HasEternitynodeVoted(const COutPoint& outpointEternitynodeIn) const
{
    return mapEternitynodeVotes.count(outpointEternitynodeIn);
}
...
bool CTxLockCandidate::HasEternitynodeVoted(const COutPoint& outpointIn, const COutPoint& outpointEternitynodeIn)
{
    std::map<COutPoint, COutPointLock>::iterator it = mapOutPointLocks.find(outpointIn);
    return it !=mapOutPointLocks.end() && it->second.HasEternitynodeVoted(outpointEternitynodeIn);
}
