...
/** 
 * Fees smaller than this (in nENT) are considered zero fee (for transaction creation)
...
int CWallet::GetRealInputSpysendRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("spysend", "GetInputSpysendRounds INSERTING %s\n", hash.ToString());
        ...
        if(nout >= wtx->vout.size())
        {
            ...
            LogPrint("spysend", "GetInputSpysendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputSpysendRounds(in2, rounds+1);
...
int CWallet::GetInputSpysendRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realSpysendRounds = GetRealInputSpysendRounds(in, 0);
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputSpysendRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nSpysendRounds;
            }
        }
    }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !(fEternityNode && pcoin->vout[i].nValue == 1000*COIN);
                } else if(coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fEternityNode) found = pcoin->vout[i].nValue != 1000*COIN; // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
    ...
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(coin_type == ONLY_DENOMINATED) {
                ...
                if(rounds < nSpysendRounds) continue;
            }
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nSpysendRoundsMin, int nSpysendRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        // eternitynode-like input should not be selected by AvailableCoins now anyway
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputSpysendRounds(vin);
            if(rounds >= nSpysendRoundsMax) continue;
            if(rounds < nSpysendRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nSpysendRoundsMin, int nSpysendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nSpysendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fEternityNode && out.tx->vout[out.i].nValue == 1000*COIN) continue; //eternitynode input
...
{
    return  nInputAmount != 0 && nInputAmount % SPYSEND_COLLATERAL == 0 && nInputAmount < SPYSEND_COLLATERAL * 5 && nInputAmount > SPYSEND_COLLATERAL;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: Spysend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - SPYSEND_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - SPYSEND_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH(CTxIn v, txCollateral.vin) {
        if(!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CSpysendPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 ENT.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough Spysend non-denominated funds for this transaction that are not equal 1000 ENT.");
                    } else {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-eternity-address
                    CScript scriptChange;
...
string CWallet::PrepareSpysendDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
    ...
    if(spySendPool.GetState() != POOL_STATUS_ERROR && spySendPool.GetState() != POOL_STATUS_SUCCESS)
        if(spySendPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the Spysend pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(spySendPool.sessionDenom, 0.1*COIN, SPYSEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PrepareSpysendDenominate - preparing spysend denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    spySendPool.SendSpysendDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
