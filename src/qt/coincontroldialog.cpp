...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::ENT;
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show eternity address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            if (nRounds >= 0 || fDebug) itemOutput->setText(COLUMN_SPYSEND_ROUNDS, strPad(QString::number(nRounds), 11, " "));
