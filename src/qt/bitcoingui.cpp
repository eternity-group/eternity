    ...
    platformStyle(platformStyle)
{
    ...
    QString windowTitle = tr("Eternity Core") + " - ";
#ifdef ENABLE_WALLET
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Eternity address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and eternity: URIs)"));
...
#ifdef ENABLE_WALLET
    QSettings settings;
    if (settings.value("fShowEternitynodesTab").toBool()) {
        eternitynodeAction = new QAction(QIcon(":/icons/" + theme + "/eternitynodes"), tr("&Eternitynodes"), this);
        eternitynodeAction->setStatusTip(tr("Browse eternitynodes"));
        eternitynodeAction->setToolTip(eternitynodeAction->statusTip());
        eternitynodeAction->setCheckable(true);
#ifdef Q_OS_MAC
        eternitynodeAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_5));
#else
        eternitynodeAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
#endif
        tabGroup->addAction(eternitynodeAction);
        connect(eternitynodeAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
        connect(eternitynodeAction, SIGNAL(triggered()), this, SLOT(gotoEternitynodePage()));
    }
    ...
    aboutAction = new QAction(QIcon(":/icons/" + theme + "/about"), tr("&About Eternity Core"), this);
    aboutAction->setStatusTip(tr("Show information about Eternity Core"));
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Eternity Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Eternity addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Eternity addresses"));
    ...
    openMNConfEditorAction = new QAction(QIcon(":/icons/" + theme + "/edit"), tr("Open &Eternitynode Configuration File"), this);
    openMNConfEditorAction->setStatusTip(tr("Open Eternitynode configuration file"));    
    ...
    openAction->setStatusTip(tr("Open a eternity: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the Eternity Core help message to get a list with possible Eternity Core command-line options"));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if (settings.value("fShowEternitynodesTab").toBool())
        {
            toolbar->addAction(eternitynodeAction);
        }
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    QSettings settings;
    if (settings.value("fShowEternitynodesTab").toBool() && eternitynodeAction) {
        eternitynodeAction->setEnabled(enabled);
    }
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Eternity Core client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::gotoEternitynodePage()
{
    QSettings settings;
    if (settings.value("fShowEternitynodesTab").toBool()) {
        eternitynodeAction->setChecked(true);
        if (walletFrame) walletFrame->gotoEternitynodePage();
    }
}
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Eternity network", "", count));
}
...
void BitcoinGUI::setAdditionalDataSyncProgress(double nSyncProgress)
{
    ...
    if(eternitynodeSync.IsBlockchainSynced())
    {
        ...
        strSyncStatus = QString(eternitynodeSync.GetSyncStatus().c_str());
...
{
    QString strTitle = tr("Eternity Core"); // default title
