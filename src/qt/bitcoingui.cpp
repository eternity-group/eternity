...
void BitcoinGUI::createActions(const NetworkStyle *networkStyle)
{
    ...
    aboutAction = new QAction(networkStyle->getAppIcon(), tr("&About Eternity Core"), this);
    ...
    optionsAction->setStatusTip(tr("Modify configuration options for Eternity"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the Eternity Core help message to get a list with possible Eternity command-line options"));
...
//    if(secs < 25*60) // 90*60 for bitcoin but we are 4x times faster
    if(eternitynodeSync.IsBlockchainSynced())
    {
        ...
        if(eternitynodeSync.IsSynced()) {
            ...
            nAttempt = eternitynodeSync.RequestedEternitynodeAttempt < ETERNITYNODE_SYNC_THRESHOLD ?
                        eternitynodeSync.RequestedEternitynodeAttempt + 1 : ETERNITYNODE_SYNC_THRESHOLD;
            progress = nAttempt + (eternitynodeSync.RequestedEternitynodeAssets - 1) * ETERNITYNODE_SYNC_THRESHOLD;
            progressBar->setMaximum(4 * ETERNITYNODE_SYNC_THRESHOLD);
