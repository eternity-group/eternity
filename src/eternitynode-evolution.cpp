...
void CEvolutionManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CEvolutionVote>::iterator it1 = mapOrphanEternitynodeEvolutionVotes.begin();
    while(it1 != mapOrphanEternitynodeEvolutionVotes.end()){
        if(evolution.UpdateProposal(((*it1).second), NULL, strError)){
            ...
            mapOrphanEternitynodeEvolutionVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
void CEvolutionManager::SubmitFinalEvolution()
{
    ...
    if(!GetTransaction(txidCollateral, txCollateral, nBlockHash, true)) {
        LogPrintf ("CEvolutionManager::SubmitFinalEvolution - Can't find collateral tx %s", txidCollateral.ToString());
        return;
    }
...
CEvolutionDB::CEvolutionDB()
{
    ...
    strMagicMessage = "EternitynodeEvolution";
}
...
bool CEvolutionManager::HasNextFinalizedEvolution()
{
    ...
    if(eternitynodeSync.IsEvolutionFinEmpty()) return true;
...
void CEvolutionManager::NewBlock()
{
    ...
    if (eternitynodeSync.RequestedEternitynodeAssets <= ETERNITYNODE_SYNC_EVOLUTION) return;
    ...
    while(it4 != vecImmatureEvolutionProposals.end())
    {
        ...
        if(!(*it4).IsValid(strError)) {
            LogPrintf("eprop (immature) - invalid evolution proposal - %s\n", strError);
        ...
        LogPrintf("eprop (immature) - new evolution - %s\n", (*it4).GetHash().ToString());
        it4 = vecImmatureEvolutionProposals.erase(it4); 
    }
...
void CEvolutionManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "envs") { //Eternitynode vote sync
        uint256 nProp;
        vRecv >> nProp;

        if(Params().NetworkID() == CBaseChainParams::MAIN){
            if(nProp == 0) {
                if(pfrom->HasFulfilledRequest("envs")) {
                    LogPrintf("envs - peer already asked me for the list\n");
                    Misbehaving(pfrom->GetId(), 20);
                    return;
                }
                pfrom->FulfilledRequest("envs");
            }
        }

        Sync(pfrom, nProp);
        LogPrintf("envs - Sent Eternitynode votes to %s\n", pfrom->addr.ToString());
    }

    if (strCommand == "eprop") { //Eternitynode Proposal
        CEvolutionProposalBroadcast evolutionProposalBroadcast;
        vRecv >> evolutionProposalBroadcast;

        if(mapSeenEternitynodeEvolutionProposals.count(evolutionProposalBroadcast.GetHash())){
            eternitynodeSync.AddedEvolutionItem(evolutionProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenEternitynodeEvolutionProposals.insert(make_pair(evolutionProposalBroadcast.GetHash(), evolutionProposalBroadcast));

        if(!evolutionProposalBroadcast.IsValid(strError)) {
            LogPrintf("eprop - invalid evolution proposal - %s\n", strError);
            return;
        }
        ...
        LogPrintf("eprop - new evolution - %s\n", evolutionProposalBroadcast.GetHash().ToString());

        //We might have active votes for this proposal that are valid now
        CheckOrphanVotes();
    }

    if (strCommand == "evote") { //Eternitynode Vote
        CEvolutionVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if(mapSeenEternitynodeEvolutionVotes.count(vote.GetHash())){
            eternitynodeSync.AddedEvolutionItem(vote.GetHash());
            return;
        }
        ...
        mapSeenEternitynodeEvolutionVotes.insert(make_pair(vote.GetHash(), vote));
        ...
        LogPrintf("evote - new evolution vote - %s\n", vote.GetHash().ToString());
    }
    ...
    if (strCommand == "fbs") { //Finalized Evolution Suggestion
        ...
        if(mapSeenFinalizedEvolutions.count(finalizedEvolutionBroadcast.GetHash())){
            eternitynodeSync.AddedEvolutionItem(finalizedEvolutionBroadcast.GetHash());
            return;
        }
...
void CEvolutionManager::ResetSync()
{
    ...
    std::map<uint256, CEvolutionProposalBroadcast>::iterator it1 = mapSeenEternitynodeEvolutionProposals.begin();
    while(it1 != mapSeenEternitynodeEvolutionProposals.end()){
...
void CEvolutionManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", ETERNITYNODE_SYNC_EVOLUTION_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", ETERNITYNODE_SYNC_EVOLUTION_FIN, nInvCount);
    LogPrintf("CEvolutionManager::Sync - sent %d items\n", nInvCount);
    ...
    if(!mapFinalizedEvolutions.count(vote.nEvolutionHash)){
        if(pfrom){
            ...
            if(!askedForSourceProposalOrEvolution.count(vote.nEvolutionHash)){
                pfrom->PushMessage("envs", vote.nEvolutionHash);
...
bool CEvolutionProposal::AddOrUpdateVote(CEvolutionVote& vote, std::string& strError)
{
    ...
    if(mapVotes.count(hash)){
        if(mapVotes[hash].nTime > vote.nTime){
            ...
            LogPrint("enevolution", "CEvolutionProposal::AddOrUpdateVote - %s\n", strError);
            return false;
        }
...
// If Eternitynode voted for a proposal, but is now invalid -- remove the vote
...
bool CEvolutionVote::Sign(CKey& keyEternitynode, CPubKey& pubKeyEternitynode)
{
    // Choose coins to use
...
bool CEvolutionVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if(pen == NULL)
    {
        LogPrint("enevolution", "CEvolutionVote::SignatureValid() - Unknown Eternitynode - %s\n", vin.ToString());
        return false;
    }
...
bool CFinalizedEvolution::AddOrUpdateVote(CFinalizedEvolutionVote& vote, std::string& strError)
{
    ...
    if(mapVotes.count(hash)){
        if(mapVotes[hash].nTime > vote.nTime){
            strError = strprintf("new vote older than existing vote - %s\n", vote.GetHash().ToString());
            LogPrint("enevolution", "CFinalizedEvolution::AddOrUpdateVote - %s\n", strError);
            return false;
        }
        if(vote.nTime - mapVotes[hash].nTime < EVOLUTION_VOTE_UPDATE_MIN){
            strError = strprintf("time between votes is too soon - %s - %lli\n", vote.GetHash().ToString(), vote.nTime - mapVotes[hash].nTime);
...
//evaluate if we should vote for this. Eternitynode only
void CFinalizedEvolution::AutoCheck()
{
    ...
    if(!fEternityNode || fAutoChecked) return;
...
void CFinalizedEvolution::SubmitVote()
{
    ...
    CFinalizedEvolutionVote vote(activeEternitynode.vin, GetHash());
...
bool CFinalizedEvolutionVote::Sign(CKey& keyEternitynode, CPubKey& pubKeyEternitynode)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedEvolutionVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if(pen == NULL)
    {
        LogPrint("enevolution", "CFinalizedEvolutionVote::SignatureValid() - Unknown Eternitynode\n");
        return false;
    }
...
std::string CEvolutionManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() <<
            ...
            ", Seen Evolutions: " << (int)mapSeenEternitynodeEvolutionProposals.size() <<
            ", Seen Evolution Votes: " << (int)mapSeenEternitynodeEvolutionVotes.size() <<
