...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "eternity.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "eternityd.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 4855, 14855) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in ENT/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in ENT/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           eternity (or specifically: spysend, instantx, eternitynode, keepass, enpayments, enevolution)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in ENT/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all Eternity specific functionality (Eternitynodes, Spysend, InstantX, Evolution) (0-1, default: %u)"), 0) + "\n";

    strUsage += "\n" + _("Eternitynode options:") + "\n";
    strUsage += "  -eternitynode=<n>            " + strprintf(_("Enable the client to act as a eternitynode (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -mnconf=<file>             " + strprintf(_("Specify eternitynode configuration file (default: %s)"), "eternitynode.conf") + "\n";
    strUsage += "  -mnconflock=<n>            " + strprintf(_("Lock eternitynodes from eternitynode configuration file (default: %u)"), 1) + "\n";
    strUsage += "  -eternitynodeprivkey=<n>     " + _("Set the eternitynode private key") + "\n";
    strUsage += "  -eternitynodeaddr=<n>        " + strprintf(_("Set external address:port to get to this eternitynode (example: %s)"), "128.127.106.235:4855") + "\n";
    ...
    strUsage += "\n" + _("Spysend options:") + "\n";
    strUsage += "  -enablespysend=<n>          " + strprintf(_("Enable use of automated spysend for funds stored in this wallet (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -spysendrounds=<n>          " + strprintf(_("Use N separate eternitynodes to anonymize funds  (2-8, default: %u)"), 2) + "\n";
    strUsage += "  -anonymizeeternityamount=<n>     " + strprintf(_("Keep N ENT anonymized (default: %u)"), 0) + "\n";
    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Spysend by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 4854, 14854) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Eternity Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Eternity is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
...
#endif
    // Make sure only a single Eternity process is using the data directory.
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Eternity version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    CEternitynodePaymentDB enpayments;
    CEternitynodePaymentDB::ReadResult readResult3 = enpayments.Read(eternitynodePayments);
    
    if (readResult3 == CEternitynodePaymentDB::FileError)
        LogPrintf("Missing eternitynode payment cache - enpayments.dat, will try to recreate\n");
    else if (readResult3 != CEternitynodePaymentDB::Ok)
    {
        LogPrintf("Error reading enpayments.dat: ");
        if(readResult3 == CEternitynodePaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }
    ...
    if(fEternityNode) {
        LogPrintf("IS SPYSEND ETERNITY NODE\n");
        strEternityNodeAddr = GetArg("-eternitynodeaddr", "");

        LogPrintf(" addr %s\n", strEternityNodeAddr.c_str());

        if(!strEternityNodeAddr.empty()){
            CService addrTest = CService(strEternityNodeAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -eternitynodeaddr address: " + strEternityNodeAddr);
            }
        }

        strEternityNodePrivKey = GetArg("-eternitynodeprivkey", "");
        if(!strEternityNodePrivKey.empty()){
            ...
            if(!spySendSigner.SetKey(strEternityNodePrivKey, errorMessage, key, pubkey))
            {
                return InitError(_("Invalid eternitynodeprivkey. Please see documenation."));
            }

            activeEternitynode.pubKeyEternitynode = pubkey;
    ...
    //get the mode of evolution voting for this eternitynode
    ...
    nAnonymizeEternityAmount = GetArg("-anonymizeeternityamount", 0);
    if(nAnonymizeEternityAmount > 999999) nAnonymizeEternityAmount = 999999;
    if(nAnonymizeEternityAmount < 2) nAnonymizeEternityAmount = 2;
    ...
    LogPrintf("Anonymize Eternity Amount %d\n", nAnonymizeEternityAmount);
    LogPrintf("Evolution Mode %s\n", strEvolutionMode.c_str());

    /* Denominations

       A note about convertability. Within Spysend pools, each denomination
