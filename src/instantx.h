...
/*
    At 15 signatures, 1/2 of the eternitynode network can be owned by
...
private:
    ...
    //track eternitynodes who voted with no txreq (for DOS protection)
    std::map<COutPoint, int64_t> mapEternitynodeOrphanVotes; // mn outpoint - time
    ...
    int64_t GetAverageEternitynodeOrphanVoteTime();
...
private:
    uint256 txHash;
    COutPoint outpoint;
    COutPoint outpointEternitynode;
...
public:
    CTxLockVote() :
        txHash(),
        outpoint(),
        outpointEternitynode(),
        vchEternitynodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}

    CTxLockVote(const uint256& txHashIn, const COutPoint& outpointIn, const COutPoint& outpointEternitynodeIn) :
        txHash(txHashIn),
        outpoint(outpointIn),
        outpointEternitynode(outpointEternitynodeIn),
        vchEternitynodeSignature(),
        nConfirmedHeight(-1),
        nTimeCreated(GetTime())
        {}
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(txHash);
        READWRITE(outpoint);
        READWRITE(outpointEternitynode);
        READWRITE(vchEternitynodeSignature);
    }
    ...
    COutPoint GetEternitynodeOutpoint() const { return outpointEternitynode; }
...
private:
    ...
    std::map<COutPoint, CTxLockVote> mapEternitynodeVotes; // eternitynode outpoint - vote
...
public:
    ...
    bool HasEternitynodeVoted(const COutPoint& outpointEternitynodeIn) const;
    int CountVotes() const { return mapEternitynodeVotes.size(); }
...
public:
    ...
    bool HasEternitynodeVoted(const COutPoint& outpointIn, const COutPoint& outpointEternitynodeIn);
