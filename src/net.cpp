...
namespace {
    ...
    const int MAX_OUTBOUND_ETERNITYNODE_CONNECTIONS = 20;
...
static CSemaphore *semEternitynodeOutbound = NULL;
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, bool fConnectToEternitynode)
{
    if (pszDest == NULL) {
        // we clean eternitynode connections in CEternitynodeMan::ProcessEternitynodeConnections()
        // so should be safe to skip this and connect to local Hot MN on CActiveEternitynode::ManageState()
        if (IsLocal(addrConnect) && !fConnectToEternitynode)
            return NULL;
        ...
        if (pnode)
        {
            // we have existing connection to this node but it was not a connection to eternitynode,
            // change flag and add reference so that we can correctly clear it later
            if(fConnectToEternitynode && !pnode->fEternitynode) {
                pnode->AddRef();
                pnode->fEternitynode = true;
            }
            return pnode;
        }
    }
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Eternity users should be well used to small numbers with many decimal places by now :)
...
static void AcceptConnection(const ListenSocket& hListenSocket) {
    ...
    if(fEternityNode && !eternitynodeSync.IsSynced()) {
        LogPrintf("AcceptConnection -- eternitynode is not synced yet, skipping inbound connection attempt\n");
        CloseSocket(hSocket);
        return;
    }
    ...
    while (true)
    {
        ...
        //
        {
            ...
            BOOST_FOREACH(CNode* pnode, vNodesCopy)
            {
                ...
                {
                    LogPrintf("ThreadSocketHandler -- removing node: peer=%d addr=%s nRefCount=%d fNetworkNode=%d fInbound=%d fEternitynode=%d\n",
                              pnode->id, pnode->addr.ToString(), pnode->GetRefCount(), pnode->fNetworkNode, pnode->fInbound, pnode->fEternitynode);
                    ...
                    pnode->grantEternitynodeOutbound.Release();
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Eternity Core " + FormatFullVersion();
...
{
    // Connecting to specific addresses, no eternitynode connections available
    if (mapArgs.count("-connect") && mapMultiArgs["-connect"].size() > 0)
        return;
    ...
    while (true)
    {
        ...
        CSemaphoreGrant grant(*semEternitynodeOutbound);
        boost::this_thread::interruption_point();
        ...
        grant.MoveTo(pnode->grantEternitynodeOutbound);
        ...
        while(it != p.second.end()) {
            if(*it != uint256()) {
                vToFetch.push_back(CInv(MSG_ETERNITYNODE_ANNOUNCE, *it));
                LogPrint("eternitynode", "ThreadMnbRequestConnections -- asking for mnb %s from addr=%s\n", it->ToString(), p.first.ToString());
            }
            ++it;
        }
...
bool BindListenPort(const CService &addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Eternity Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if (semEternitynodeOutbound == NULL) {
        // initialize semaphore
        semEternitynodeOutbound = new CSemaphore(MAX_OUTBOUND_ETERNITYNODE_CONNECTIONS);
    }
    ...
    // Initiate eternitynode connections
...
bool StopNode()
{
    ...
    if (semEternitynodeOutbound)
        for (int i=0; i<MAX_OUTBOUND_ETERNITYNODE_CONNECTIONS; i++)
            semEternitynodeOutbound->post();
...
public:
    ...
    ~CNetCleanup()
    {
        ...
        delete semOutbound;
        semOutbound = NULL;
        delete semEternitynodeOutbound;
        semEternitynodeOutbound = NULL;
