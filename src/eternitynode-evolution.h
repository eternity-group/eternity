...
#ifndef ETERNITYNODE_EVOLUTION_H
#define ETERNITYNODE_EVOLUTION_H
...
public:
    ...
    std::map<uint256, CEvolutionProposalBroadcast> mapSeenEternitynodeEvolutionProposals;
    std::map<uint256, CEvolutionVote> mapSeenEternitynodeEvolutionVotes;
    std::map<uint256, CEvolutionVote> mapOrphanEternitynodeEvolutionVotes;
    ...
    void ClearSeen() {
        mapSeenEternitynodeEvolutionProposals.clear();
        mapSeenEternitynodeEvolutionVotes.clear();
    ...
    void Clear(){
        ...
        mapSeenFinalizedEvolutions.clear();
        mapSeenFinalizedEvolutionVotes.clear();
        mapOrphanEternitynodeEvolutionVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(mapSeenEternitynodeEvolutionProposals);
        READWRITE(mapSeenEternitynodeEvolutionVotes);
        READWRITE(mapSeenFinalizedEvolutions);
        READWRITE(mapSeenFinalizedEvolutionVotes);
        READWRITE(mapOrphanEternitynodeEvolutionVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (eternitynode only)
...
public:
    ...
    void AutoCheck();
    //total eternity paid out by this evolution
    CAmount GetTotalPayout();
    //vote on this finalized evolution as a eternitynode
    void SubmitVote();
...
//
// CFinalizedEvolutionVote - Allow a eternitynode node to vote and broadcast throughout the network
//
...
//
// Evolution Proposal : Contains the eternitynode votes for each evolution
//
...
//
// CEvolutionVote - Allow a eternitynode node to vote and broadcast throughout the network
//
