...
UniValue getblocktemplate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "  \"eternitynode\" : {                  (json object) required eternitynode payee that must be included in the next block\n"
            "      \"payee\" : \"xxxx\",             (string) payee address\n"
            "      \"script\" : \"xxxx\",            (string) payee scriptPubKey\n"
            "      \"amount\": n                   (numeric) required amount to pay\n"
            "  },\n"
            "  \"eternitynode_payments_started\" :  true|false, (boolean) true, if eternitynode payments started\n"
            "  \"eternitynode_payments_enforced\" : true|false, (boolean) true, if eternitynode payments are enforced\n"
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Eternity Core is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Eternity Core is downloading blocks...");

    if (!eternitynodeSync.IsSynced())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Eternity Core is syncing with network...");
    ...
    UniValue eternitynodeObj(UniValue::VOBJ);
    if(pblock->txoutEternitynode != CTxOut()) {
        CTxDestination address1;
        ExtractDestination(pblock->txoutEternitynode.scriptPubKey, address1);
        CBitcoinAddress address2(address1);
        eternitynodeObj.push_back(Pair("payee", address2.ToString().c_str()));
        eternitynodeObj.push_back(Pair("script", HexStr(pblock->txoutEternitynode.scriptPubKey.begin(), pblock->txoutEternitynode.scriptPubKey.end())));
        eternitynodeObj.push_back(Pair("amount", pblock->txoutEternitynode.nValue));
    }
    result.push_back(Pair("eternitynode", eternitynodeObj));
