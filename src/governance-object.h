...
private:
    ...
    /// Eternitynode info for signed objects
    ...
    // VARIOUS FLAGS FOR OBJECT / SET VIA ETERNITYNODE VOTING
...
public:
    ...
    const CTxIn& GetEternitynodeVin() const {
        return vinEternitynode;
    }
    ...
    void SetEternitynodeInfo(const CTxIn& vin);
    bool Sign(CKey& keyEternitynode, CPubKey& pubKeyEternitynode);
    bool CheckSignature(CPubKey& pubKeyEternitynode);
    ...
    bool IsValidLocally(std::string& strError, bool& fMissingEternitynode, bool fCheckCollateral);
