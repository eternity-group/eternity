...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "\nStop Eternity server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "Eternity server stopping";
}
...
{ //  category              name                      actor (function)         okSafeMode threadSafe reqWallet
  //  --------------------- ------------------------  -----------------------  ---------- ---------- ---------
    ...
    /* Eternity features */
    { "eternity",               "eternitynode",             &eternitynode,             true,      true,       false },
    { "eternity",               "eternitynodelist",         &eternitynodelist,         true,      true,       false },
    { "eternity",               "eternitynodebroadcast",    &eternitynodebroadcast,    true,      true,       false },
    { "eternity",               "enevolution",               &enevolution,               true,      true,       false },
    { "eternity",               "enevolutionvoteraw",        &enevolutionvoteraw,        true,      true,       false },
    { "eternity",               "mnfinalevolution",          &mnfinalevolution,          true,      true,       false },
    { "eternity",               "mnsync",                 &mnsync,                 true,      true,       false },
    { "eternity",               "spork",                  &spork,                  true,      true,       false },
#ifdef ENABLE_WALLET
    { "eternity",               "spysend",               &spysend,               false,     false,      true  }, /* not threadSafe because of SendMoney */
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use eternityd, or the -server option to eternity-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
...
std::string HelpExampleCli(string methodname, string args){
    return "> eternity-cli " + methodname + " " + args + "\n";
}

std::string HelpExampleRpc(string methodname, string args){
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
        "\"method\": \"" + methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:4854/\n";
}
