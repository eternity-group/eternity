...
CGovernanceVote::CGovernanceVote(CTxIn vinEternitynodeIn, uint256 nParentHashIn, vote_signal_enum_t eVoteSignalIn, vote_outcome_enum_t eVoteOutcomeIn)
    : fValid(true),
      fSynced(false),
      nVoteSignal(eVoteSignalIn),
      vinEternitynode(vinEternitynodeIn),
...
bool CGovernanceVote::Sign(CKey& keyEternitynode, CPubKey& pubKeyEternitynode)
{
    // Choose coins to use
    ...
    std::string strError;
    std::string strMessage = vinEternitynode.prevout.ToStringShort() + "|" + nParentHash.ToString() + "|" +
...
bool CGovernanceVote::IsValid(bool fSignatureCheck) const
{
    ...
    if(!infoMn.fInfoValid) {
        LogPrint("gobject", "CGovernanceVote::IsValid -- Unknown Eternitynode - %s\n", vinEternitynode.prevout.ToStringShort());
        return false;
    }
    ...
    if(!spySendSigner.VerifyMessage(infoMn.pubKeyEternitynode, vchSig, strMessage, strError)) {
...
{
    bool fResult = ((vote1.vinEternitynode == vote2.vinEternitynode) &&
...
bool operator<(const CGovernanceVote& vote1, const CGovernanceVote& vote2)
{
    ...
    fResult = (vote1.vinEternitynode == vote2.vinEternitynode);
