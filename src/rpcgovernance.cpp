...
UniValue gobject(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-alias         - Vote on a governance object by eternitynode alias (using eternitynode.conf setup)\n"
                "  vote-conf          - Vote on a governance object by eternitynode configured in eternity.conf\n"
                "  vote-many          - Vote on a governance object by all eternitynodes (using eternitynode.conf setup)\n"
                );
    ...
    */
    ...
    if(strCommand == "prepare")
    {
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Trigger and watchdog objects need not be prepared (however only eternitynodes can create them)");
        }
    ...
    if(strCommand == "submit")
    {
        ...
        if(!eternitynodeSync.IsBlockchainSynced()) {
            throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Must wait for client to sync with eternitynode network. Try again in a minute or so.");
        }

        CEternitynode mn;
        bool fMnFound = mnodeman.Get(activeEternitynode.vin, mn);

        DBG( cout << "gobject: submit activeEternitynode.pubKeyEternitynode = " << activeEternitynode.pubKeyEternitynode.GetHash().ToString()
             << ", vin = " << activeEternitynode.vin.prevout.ToStringShort()
        ...
        if((govobj.GetObjectType() == GOVERNANCE_OBJECT_TRIGGER) ||
           (govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG)) {
            if(fMnFound) {
                govobj.SetEternitynodeInfo(mn.vin);
                govobj.Sign(activeEternitynode.keyEternitynode, activeEternitynode.pubKeyEternitynode);
            }
            else {
                LogPrintf("gobject(submit) -- Object submission rejected because node is not a eternitynode\n");
                throw JSONRPCError(RPC_INVALID_PARAMETER, "Only valid eternitynodes can submit this type of object");
            }
        }
        else {
            if(params.size() != 6) {
        ...
        if(!governance.EternitynodeRateCheck(govobj)) {
        ...
        if(!governance.EternitynodeRateCheck(govobj, UPDATE_TRUE)) {
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            statusObj.push_back(Pair("errorMessage", "Can't find eternitynode by collateral output"));
            resultsObj.push_back(Pair("eternity.conf", statusObj));
        ...
        if(!vote.Sign(activeEternitynode.keyEternitynode, activeEternitynode.pubKeyEternitynode)) {
            nFailed++;
            statusObj.push_back(Pair("result", "failed"));
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CEternitynodeConfig::CEternitynodeEntry mne, eternitynodeConfig.getEntries()) {
            ...
            if(!spySendSigner.GetKeysFromSecret(mne.getPrivKey(), keyEternitynode, pubKeyEternitynode)){
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Eternitynode signing error, could not set key correctly"));
    ...
    // ETERNITYNODES CAN VOTE ON GOVERNANCE OBJECTS ON THE NETWORK FOR VARIOUS SIGNALS AND OUTCOMES
    if(strCommand == "vote-alias")
    {
        ...
        // EXECUTE VOTE FOR EACH ETERNITYNODE, COUNT SUCCESSES VS FAILURES
        ...
        BOOST_FOREACH(CEternitynodeConfig::CEternitynodeEntry mne, eternitynodeConfig.getEntries())
        {
            ...
            // SETUP THE SIGNING KEY FROM ETERNITYNODE.CONF ENTRY
            ...
            if(!spySendSigner.GetKeysFromSecret(mne.getPrivKey(), keyEternitynode, pubKeyEternitynode)) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", strprintf("Invalid eternitynode key %s.", mne.getPrivKey())));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }

            // SEARCH FOR THIS ETERNITYNODE ON THE NETWORK, THE NODE MUST BE ACTIVE TO VOTE
            ...
            if(!fMnFound) {
                nFailed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Eternitynode must be publically available on network to vote. Eternitynode not found."));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
    ...
    if(strCommand == "list" || strCommand == "diff")
    {
        ...
        BOOST_FOREACH(CGovernanceObject* pGovObj, objs)
        {
            ...
            const CTxIn& eternitynodeVin = pGovObj->GetEternitynodeVin();
            if(eternitynodeVin != CTxIn()) {
                bObj.push_back(Pair("SigningEternitynode", eternitynodeVin.prevout.ToStringShort()));
            }
    ...
    if(strCommand == "get")
    {
        ...
        const CTxIn& eternitynodeVin = pGovObj->GetEternitynodeVin();
        if(eternitynodeVin != CTxIn()) {
            objResult.push_back(Pair("SigningEternitynode", eternitynodeVin.prevout.ToStringShort()));
        }
        ...
        // -- ENDORSED VIA ETERNITYNODE-ELECTED BOARD
    ...
    if(strCommand == "getcurrentvotes")
    {
        ...
        if (params.size() == 4) {
            uint256 txid = ParseHashV(params[2], "Eternitynode Collateral hash");
...
UniValue voteraw(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
                "voteraw <eternitynode-tx-hash> <eternitynode-tx-index> <governance-hash> <vote-signal> [yes|no|abstain] <time> <vote-sig>\n"
    ...
    if(!fMnFound) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failure to find eternitynode in list : " + vin.prevout.ToStringShort());
    }
...
UniValue getgovernanceinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0) {
        throw std::runtime_error(
            ...
            "  \"eternitynodewatchdogmaxseconds\": xxxxx,  (numeric) sentinel watchdog expiration time in seconds\n"
    ...
    obj.push_back(Pair("eternitynodewatchdogmaxseconds", ETERNITYNODE_WATCHDOG_MAX_SECONDS));
