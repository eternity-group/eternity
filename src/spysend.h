...
// timeouts
static const int SPYSEND_AUTO_TIMEOUT_MIN       = 5;
static const int SPYSEND_AUTO_TIMEOUT_MAX       = 15;
static const int SPYSEND_QUEUE_TIMEOUT          = 30;
static const int SPYSEND_SIGNING_TIMEOUT        = 15;

//! minimum peer version accepted by mixing pool
static const int MIN_SPYSEND_PEER_PROTO_VERSION = 70206;

static const CAmount SPYSEND_COLLATERAL         = 0.001 * COIN;
static const CAmount SPYSEND_POOL_MAX           = 999.999 * COIN;
static const int DENOMS_COUNT_MAX                   = 100;

static const int DEFAULT_SPYSEND_ROUNDS         = 2;
static const int DEFAULT_SPYSEND_AMOUNT         = 1000;
static const int DEFAULT_SPYSEND_LIQUIDITY      = 0;
static const bool DEFAULT_SPYSEND_MULTISESSION  = false;

// Warn user if mixing in gui or try to create backup if mixing in daemon mode
// when we have only this many keys left
static const int SPYSEND_KEYS_THRESHOLD_WARNING = 100;
// Stop mixing completely, it's too dangerous to continue when we have only this many keys left
static const int SPYSEND_KEYS_THRESHOLD_STOP    = 50;
...
public:
    ...
    bool CheckSignature(const CPubKey& pubKeyEternitynode);

    bool Relay();

    /// Is this queue expired?
    bool IsExpired() { return GetTime() - nTime > SPYSEND_QUEUE_TIMEOUT; }

    std::string ToString()
    {
        return strprintf("nDenom=%d, nTime=%lld, fReady=%s, fTried=%s, eternitynode=%s",
...
{
public:
    /// Is the input associated with this public key? (and there is 1000 ENT - checking if valid eternitynode)
...
private:
    ...
    std::vector<CSpySendEntry> vecEntries; // Eternitynode/clients entries
    ...
    /// As a client, submit part of a future mixing transaction to a Eternitynode to start the process
    ...
    /// Get Eternitynode updates about the progress of mixing
