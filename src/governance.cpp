...
void CGovernanceManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
...

    {
        ...
        if(!eternitynodeSync.IsEternitynodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECT -- eternitynode list not synced\n");
            return;
        }
        ...
        if(!EternitynodeRateCheck(govobj, UPDATE_FAIL_ONLY, false, fRateCheckBypassed)) {
            LogPrintf("MNGOVERNANCEOBJECT -- eternitynode rate check failed - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
            return;
        }
        ...
        bool fEternitynodeMissing = false;
        bool fIsValid = govobj.IsValidLocally(strError, fEternitynodeMissing, true);

        if(fEternitynodeMissing) {
            mapEternitynodeOrphanObjects.insert(std::make_pair(nHash, object_time_pair_t(govobj, GetAdjustedTime() + GOVERNANCE_ORPHAN_EXPIRATION_TIME)));
            LogPrintf("MNGOVERNANCEOBJECT -- Missing eternitynode for: %s, strError = %s\n", strHash, strError);
        ...
        if(fRateCheckBypassed) {
            if(!EternitynodeRateCheck(govobj, UPDATE_FAIL_ONLY, true, fRateCheckBypassed)) {
                LogPrintf("MNGOVERNANCEOBJECT -- eternitynode rate check failed (after signature verification) - %s - (current block height %d) \n", strHash, nCachedBlockHeight);
                return;
            }
        }
        ...
        if(fAddToSeen) {
            ...
            EternitynodeRateCheck(govobj, UPDATE_TRUE, true, fRateCheckBypassed);
        }

        eternitynodeSync.AddedGovernanceItem();
    ...
    {
        // Ignore such messages until eternitynode list is synced
        if(!eternitynodeSync.IsEternitynodeListSynced()) {
            LogPrint("gobject", "MNGOVERNANCEOBJECTVOTE -- eternitynode list not synced\n");
            return;
        }
        ...
        if(ProcessVote(pfrom, vote, exception)) {
            ...
            if((exception.GetNodePenalty() != 0) && eternitynodeSync.IsSynced()) {
...
void CGovernanceManager::UpdateCachesAndClean()
{
    ...
    for(size_t i = 0; i < vecDirtyHashes.size(); ++i) {
        ...
        it->second.ClearEternitynodeVotes();
...
std::vector<CGovernanceVote> CGovernanceManager::GetCurrentVotes(const uint256& nParentHash, const CTxIn& mnCollateralOutpointFilter)
{
    ...
    // Compile a list of Eternitynode collateral outpoints for which to get votes
    std::vector<CTxIn> vecMNTxIn;
    if (mnCollateralOutpointFilter == CTxIn()) {
        std::vector<CEternitynode> mnlist = mnodeman.GetFullEternitynodeVector();
        for (std::vector<CEternitynode>::iterator it = mnlist.begin(); it != mnlist.end(); ++it)
        {
...
void CGovernanceManager::Sync(CNode* pfrom, const uint256& nProp, const CBloomFilter& filter)
{
    ...
    */
    ...
    if(fEternityNode && !eternitynodeSync.IsSynced()) return;
    ...
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, ETERNITYNODE_SYNC_GOVOBJ, nObjCount);
    pfrom->PushMessage(NetMsgType::SYNCSTATUSCOUNT, ETERNITYNODE_SYNC_GOVOBJ_VOTE, nVoteCount);
    LogPrintf("CGovernanceManager::Sync -- sent %d objects and %d votes to peer=%d\n", nObjCount, nVoteCount, pfrom->id);
}

bool CGovernanceManager::EternitynodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast)
{
    bool fRateCheckBypassed = false;
    return EternitynodeRateCheck(govobj, eUpdateLast, true, fRateCheckBypassed);
}

bool CGovernanceManager::EternitynodeRateCheck(const CGovernanceObject& govobj, update_mode_enum_t eUpdateLast, bool fForce, bool& fRateCheckBypassed)
{
    LOCK(cs);
    ...
    const CTxIn& vin = govobj.GetEternitynodeVin();

    txout_m_it it  = mapLastEternitynodeObject.find(vin.prevout);

    if(it == mapLastEternitynodeObject.end()) {
        if(eUpdateLast == UPDATE_TRUE) {
            it = mapLastEternitynodeObject.insert(txout_m_t::value_type(vin.prevout, last_object_rec(true))).first;
            switch(nObjectType) {
            case GOVERNANCE_OBJECT_TRIGGER:
    ...
    if(nTimestamp < nNow - 2 * nSuperblockCycleSeconds) {
        LogPrintf("CGovernanceManager::EternitynodeRateCheck -- object %s rejected due to too old timestamp, eternitynode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }

    if(nTimestamp > nNow + 60*60) {
        LogPrintf("CGovernanceManager::EternitynodeRateCheck -- object %s rejected due to too new (future) timestamp, eternitynode vin = %s, timestamp = %d, current time = %d\n",
                 strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    if(!pBuffer) {
        LogPrintf("CGovernanceManager::EternitynodeRateCheck -- Internal Error returning false, NULL ptr found for object %s eternitynode vin = %s, timestamp = %d, current time = %d\n",
                  strHash, vin.prevout.ToStringShort(), nTimestamp, nNow);
        return false;
    }
    ...
    if(fRateOK) {
        ...
        LogPrintf("CGovernanceManager::EternitynodeRateCheck -- Rate too high: object hash = %s, eternitynode vin = %s, object timestamp = %d, rate = %f, max rate = %f\n",
...
bool CGovernanceManager::ProcessVote(CNode* pfrom, const CGovernanceVote& vote, CGovernanceException& exception)
{
    ...
    if(mapInvalidVotes.HasKey(nHashVote)) {
        ...
        ostr << "CGovernanceManager::ProcessVote -- Old invalid vote "
                << ", MN outpoint = " << vote.GetVinEternitynode().prevout.ToStringShort()
    ...
    if(fOk) {
        ...
        if(govobj.GetObjectType() == GOVERNANCE_OBJECT_WATCHDOG) {
            mnodeman.UpdateWatchdogVoteTime(vote.GetVinEternitynode());
        }
    }
    return fOk;
}

void CGovernanceManager::CheckEternitynodeOrphanVotes()
{
    LOCK2(cs_main, cs);
    fRateChecksEnabled = false;
    for(object_m_it it = mapObjects.begin(); it != mapObjects.end(); ++it) {
...
void CGovernanceManager::CheckEternitynodeOrphanObjects()
{
    LOCK2(cs_main, cs);
    int64_t nNow = GetAdjustedTime();
    fRateChecksEnabled = false;
    object_time_m_it it = mapEternitynodeOrphanObjects.begin();
    while(it != mapEternitynodeOrphanObjects.end()) {
        object_time_pair_t& pair = it->second;
        CGovernanceObject& govobj = pair.first;

        if(pair.second < nNow) {
            mapEternitynodeOrphanObjects.erase(it++);
            continue;
        }
        ...
        if(AddGovernanceObject(govobj, fAddToSeen)) {
            LogPrintf("CGovernanceManager::CheckEternitynodeOrphanObjects -- %s new\n", govobj.GetHash().ToString());
            govobj.Relay();
            mapEternitynodeOrphanObjects.erase(it++);
        }
        else {
...
int CGovernanceManager::RequestGovernanceObjectVotes(const std::vector<CNode*>& vNodesCopy)
{
    ...
    // Testnet is ~40 times smaller in eternitynode count, but only ~1000 eternitynodes usually vote,
    // so 1 obj on mainnet == ~10 objs or ~1000 votes on testnet. However we want to test a higher
    // number of votes to make sure it's robust enough, so aim at 2000 votes per eternitynode per request.
    ...
    for (int i = 0; i < nMaxObjRequestsPerNode; ++i) {
        ...
        BOOST_FOREACH(CNode* pnode, vNodesCopy) {
            // Only use reqular peers, don't try to ask from outbound "eternitynode" connections -
            // they stay connected for a short period of time and it's possible that we won't get everything we should.
            // Only use outbound connections - inbound connection could be a "eternitynode" connection
...
int CGovernanceManager::GetEternitynodeIndex(const CTxIn& eternitynodeVin)
{
    LOCK(cs);
    bool fIndexRebuilt = false;
    int nMNIndex = mnodeman.GetEternitynodeIndex(eternitynodeVin, fIndexRebuilt);
    if(fIndexRebuilt) {
        RebuildVoteMaps();
        nMNIndex = mnodeman.GetEternitynodeIndex(eternitynodeVin, fIndexRebuilt);
        if(fIndexRebuilt) {
            LogPrintf("CGovernanceManager::GetEternitynodeIndex -- WARNING: vote map rebuild failed\n");
        }
    }
    return nMNIndex;
}
...
void CGovernanceManager::RebuildVoteMaps()
{
    ...
    mnodeman.ClearOldEternitynodeIndex();
}
...
void CGovernanceManager::InitOnLoad()
{
    ...
    LogPrintf("Preparing eternitynode indexes and governance triggers...\n");
    RebuildIndexes();
    AddCachedTriggers();
    LogPrintf("Eternitynode indexes and governance triggers prepared  %dms\n", GetTimeMillis() - nStart);
    LogPrintf("     %s\n", ToString());
}
