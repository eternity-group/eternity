...
#if defined(NDEBUG)
# error "Eternity cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in nENT) are considered zero fee (for relaying and mining)
...
const string strMessageMagic = "Eternity Signed Message:\n";
...
int64_t GetEternitynodePayment(int nHeight, int64_t blockValue)
{
    int64_t ret = 0;
    ret =  blockValue/2; // 50% PoS && 50% PoW
    return ret;
}
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ----------- eternitynode payments / evolutions -----------
    ...
    if(pindexPrev != NULL)
    {
        ...
        if(nHeight != 0){
            ...
            {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(100, error("CheckBlock() : Couldn't find eternitynode/evolution payment"));
            }
        } else {
...
bool ProcessNewBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if(!fLiteMode){
        if (eternitynodeSync.RequestedEternitynodeAssets > ETERNITYNODE_SYNC_LIST) {
            spySendPool.NewBlock();
            eternitynodePayments.ProcessBlock(GetHeight()+10);
            evolution.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_ETERNITYNODE_WINNER:
        if(eternitynodePayments.mapEternitynodePayeeVotes.count(inv.hash)) {
            eternitynodeSync.AddedEternitynodeWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_EVOLUTION_VOTE:
        if(evolution.mapSeenEternitynodeEvolutionVotes.count(inv.hash)) {
            eternitynodeSync.AddedEvolutionItem(inv.hash);
            return true;
        }
        return false;
    case MSG_EVOLUTION_PROPOSAL:
        if(evolution.mapSeenEternitynodeEvolutionProposals.count(inv.hash)) {
            eternitynodeSync.AddedEvolutionItem(inv.hash);
            return true;
        }
        return false;
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_ETERNITYNODE_WINNER) {
                    if(eternitynodePayments.mapEternitynodePayeeVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << eternitynodePayments.mapEternitynodePayeeVotes[inv.hash];
                        pfrom->PushMessage("enw", ss);
                        pushed = true;
                    }
                }
                if (!pushed && inv.type == MSG_EVOLUTION_VOTE) {
                    if(evolution.mapSeenEternitynodeEvolutionVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << evolution.mapSeenEternitynodeEvolutionVotes[inv.hash];
                        pfrom->PushMessage("evote", ss);
                        pushed = true;
                    }
                }
                ...
                if (!pushed && inv.type == MSG_EVOLUTION_PROPOSAL) {
                    if(evolution.mapSeenEternitynodeEvolutionProposals.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << evolution.mapSeenEternitynodeEvolutionProposals[inv.hash];
                        pfrom->PushMessage("eprop", ss);
                        pushed = true;
                    }
                }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        //eternitynode signed transaction
        ...
        if(strCommand == "tx") {
            ...
            {
                if(!pen->allowFreeTx){
                    //multiple peers can send us a valid eternitynode transaction
                    if(fDebug) LogPrintf("dstx: Eternitynode sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
                ...
                if(!spySendSigner.VerifyMessage(pen->pubkey2, vchSig, strMessage, errorMessage)){
                    LogPrintf("dstx: Got bad eternitynode address signature %s \n", vin.ToString());
