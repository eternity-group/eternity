...
#if defined(NDEBUG)
# error "Eternity Core cannot be compiled without assertions."
#endif
...
CAmount GetEternitynodePayment(int nHeight, CAmount blockValue)
{
...
void ThreadScriptCheck() {
    RenameThread("eternity-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // ETERNITY : MODIFIED TO CHECK ETERNITYNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ETERNITY): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ETERNITY): couldn't find eternitynode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ETERNITY : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ETERNITY): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ETERNITY): spork is off, skipping transaction locking checks\n");
    }
...
bool ProcessNewBlock(CValidationState& state, const CChainParams& chainparams, const CNode* pfrom, const CBlock* pblock, bool fForceProcessing, CDiskBlockPos* dbp)
{
    ...
    eternitynodeSync.IsBlockchainSynced(true);
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Eternity Related Inventory Messages
    ...
    */
    ...
    case MSG_ETERNITYNODE_PAYMENT_VOTE:
        return enpayments.mapEternitynodePaymentVotes.count(inv.hash);

    case MSG_ETERNITYNODE_PAYMENT_BLOCK:
        {
            BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
            return mi != mapBlockIndex.end() && enpayments.mapEternitynodeBlocks.find(mi->second->nHeight) != enpayments.mapEternitynodeBlocks.end();
        }

    case MSG_ETERNITYNODE_ANNOUNCE:
        return mnodeman.mapSeenEternitynodeBroadcast.count(inv.hash) && !mnodeman.IsMnbRecoveryRequested(inv.hash);
    ...
    case MSG_ETERNITYNODE_VERIFY:
        return mnodeman.mapSeenEternitynodeVerification.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        LogPrint("net", "ProcessGetData -- inv = %s\n", inv.ToString());
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_ETERNITYNODE_PAYMENT_VOTE) {
                    if(enpayments.HasVerifiedPaymentVote(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << enpayments.mapEternitynodePaymentVotes[inv.hash];
                        pfrom->PushMessage(NetMsgType::ETERNITYNODEPAYMENTVOTE, ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_ETERNITYNODE_PAYMENT_BLOCK) {
                    BlockMap::iterator mi = mapBlockIndex.find(inv.hash);
                    LOCK(cs_mapEternitynodeBlocks);
                    if (mi != mapBlockIndex.end() && enpayments.mapEternitynodeBlocks.count(mi->second->nHeight)) {
                        BOOST_FOREACH(CEternitynodePayee& payee, enpayments.mapEternitynodeBlocks[mi->second->nHeight].vecPayees) {
                            std::vector<uint256> vecVoteHashes = payee.GetVoteHashes();
                            BOOST_FOREACH(uint256& hash, vecVoteHashes) {
                                if(enpayments.HasVerifiedPaymentVote(hash)) {
                                    CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                                    ss.reserve(1000);
                                    ss << enpayments.mapEternitynodePaymentVotes[hash];
                                    pfrom->PushMessage(NetMsgType::ETERNITYNODEPAYMENTVOTE, ss);
                                }
                            }
                        }
                ...
                if (!pushed && inv.type == MSG_ETERNITYNODE_ANNOUNCE) {
                    if(mnodeman.mapSeenEternitynodeBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenEternitynodeBroadcast[inv.hash].second;
                ...
                if (!pushed && inv.type == MSG_ETERNITYNODE_VERIFY) {
                    if(mnodeman.mapSeenEternitynodeVerification.count(inv.hash)) {
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << mnodeman.mapSeenEternitynodeVerification[inv.hash];
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == NetMsgType::TX || strCommand == NetMsgType::DSTX || strCommand == NetMsgType::TXLOCKREQUEST)
    {
        ...
        if (strCommand == NetMsgType::TXLOCKREQUEST) {
            ...
            if(mapSpysendBroadcastTxes.count(hashTx)) {
                LogPrint("spysend", "DSTX -- Already have %s, skipping...\n", hashTx.ToString());
                return true; // not an error
            }

            CEternitynode* pmn = mnodeman.Find(dstx.vin);
            if(pmn == NULL) {
                LogPrint("spysend", "DSTX -- Can't find eternitynode %s to verify %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return false;
            }

            if(!pmn->fAllowMixingTx) {
                LogPrint("spysend", "DSTX -- Eternitynode %s is sending too many transactions %s\n", dstx.vin.prevout.ToStringShort(), hashTx.ToString());
                return true;
                // TODO: Not an error? Could it be that someone is relaying old DSTXes
                // we have no idea about (e.g we were offline)? How to handle them?
            }

            if(!dstx.CheckSignature(pmn->pubKeyEternitynode)) {
                LogPrint("spysend", "DSTX -- CheckSignature() failed for %s\n", hashTx.ToString());
                return false;
            }

            LogPrintf("DSTX -- Got Eternitynode transaction %s\n", hashTx.ToString());
        ...
        if (!AlreadyHave(inv) && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs))
        {
            ...
            if (strCommand == NetMsgType::DSTX) {
                LogPrintf("DSTX -- Eternitynode transaction accepted, txid=%s, peer=%d\n",
    ...
    else
    {
        ...
        if (found)
        {
            ...
            enpayments.ProcessMessage(pfrom, strCommand, vRecv);
            instantsend.ProcessMessage(pfrom, strCommand, vRecv);
            sporkManager.ProcessSpork(pfrom, strCommand, vRecv);
            eternitynodeSync.ProcessMessage(pfrom, strCommand, vRecv);
