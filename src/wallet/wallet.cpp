...
int CWallet::GetRealInputSpySendRounds(CTxIn txin, int nRounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        if (mdwi == mDenomWtxes.end()) {
            ...
            LogPrint("spysend", "GetRealInputSpySendRounds INSERTING %s\n", hash.ToString());
        ...
        if (nout >= wtx->vout.size()) {
            ...
            LogPrint("spysend", "GetRealInputSpySendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
...
CAmount CWallet::GetAnonymizableBalance(bool fSkipDenominated) const
{
    ...
    BOOST_FOREACH(CompactTallyItem& item, vecTally) {
        ...
        // assume that the fee to create denoms be SPYSEND_COLLATERAL at max
        if(item.nAmount >= vecSpySendDenominations.back() + (fIsDenominated ? 0 : SPYSEND_COLLATERAL))
            nTotal += item.nAmount;
    }
...
CAmount CWallet::GetNeedsToBeAnonymizedBalance(CAmount nMinBalance) const
{
    ...
    if (nNeedsToAnonymizeBalance > SPYSEND_POOL_MAX) nNeedsToAnonymizeBalance = SPYSEND_POOL_MAX;
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                } else if(nCoinType == ONLY_SPYSEND_COLLATERAL) {
...
bool CWallet::SelectCoinsGrouppedByAddresses(std::vector<CompactTallyItem>& vecTallyRet, bool fSkipDenominated, bool fAnonymizable) const
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
        ...
        for (unsigned int i = 0; i < wtx.vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fEternityNode && wtx.vout[i].nValue == 1000*COIN) continue;
    ...
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, CompactTallyItem)& item, mapTally) {
        if(fAnonymizable && item.second.nAmount < vecSpySendDenominations.back()) continue;
...
bool CWallet::GetEternitynodeVinAndKeys(CTxIn& txinRet, CPubKey& pubKeyRet, CKey& keyRet, std::string strTxHash, std::string strOutputIndex)
{
    ...
    if(vPossibleCoins.empty()) {
        LogPrintf("CWallet::GetEternitynodeVinAndKeys -- Could not locate any valid eternitynode vin\n");
        return false;
    }
    ...
    LogPrintf("CWallet::GetEternitynodeVinAndKeys -- Could not locate specified eternitynode vin\n");
    return false;
}
...
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, fOnlyConfirmed, NULL, false, ONLY_SPYSEND_COLLATERAL);
...
{
    // collateral inputs should always be a 2x..4x of SPYSEND_COLLATERAL
    return  nInputAmount >= SPYSEND_COLLATERAL * 2 &&
            nInputAmount <= SPYSEND_COLLATERAL * 4 &&
            nInputAmount %  SPYSEND_COLLATERAL == 0;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    CTxOut txout = CTxOut(nValue - SPYSEND_COLLATERAL, scriptChange);
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    ...
                    if (fUseInstantSend) {
                        if (nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                            strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 ENT."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                        } else {
