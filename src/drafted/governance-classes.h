 ...
 *
 *   // network
 *   CEternityNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.eternity-group.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CEternityNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CEternityNetwork(UniValue objIn)
    {
...
// // can be under: EternityNetwork
// //   -- signature requirements : Key1(User)
// class CEternityNetworkVariable : public CGovernanceObject
// {
// private:
